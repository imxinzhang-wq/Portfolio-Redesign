          ...
          {MOCK_PROJECTS.map((project, i) => (
            ...
            >
              <div className="flex flex-col max-w-4xl">
                {/* Title */}
                <h3 className="text-5xl md:text-7xl lg:text-[5.5rem] font-sans font-medium tracking-tight md:tracking-tighter leading-[1.05] mb-8 group-hover:text-[#1A1A1A]/70 transition-colors duration-300">
                  {project.title}
                </h3>
                
                {/* Meta & View */}
                <div className="flex flex-col gap-6 mb-16">
                  <span className="text-[11px] font-sans text-[#1A1A1A]/60 uppercase tracking-widest font-medium">
                    {project.category}
                  </span>
                  
                  <span className="inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.15em] text-[#1A1A1A] group-hover:text-[#1A1A1A]/50 transition-colors w-fit">
                    VIEW <ArrowUpRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                  </span>
                </div>
                
                {/* Description */}
                <p className="text-[17px] md:text-[19px] text-[#1A1A1A]/60 font-light leading-[1.6] max-w-lg mb-16">
                  {project.description}
                </p>
              </div>

              {/* Image Area */}
              <div className="w-full aspect-[4/3] md:aspect-[2.2/1] overflow-hidden bg-[#1A1A1A]/5">
                <img 
                  src={project.image} 
                  alt={project.title}

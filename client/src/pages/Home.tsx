...
/*
  Snapping — four stops only: each project frame centred, and the gallery at
  the point where its wordmark reads centred.

  SNAP_RADIUS_PX is how near you have to come to rest for the page to close
  the gap. It is large on purpose: a small radius was the "要贴的很近才吸附"
  complaint, and a large one is only affordable because the travel is our own
  animation rather than the browser's.

  SNAP_MS / snapEase are that animation. The page is at rest when it starts
  and at rest when it ends, so the curve is eased at both ends — an ease-out
  would leave at full speed from a standstill, which is precisely what reads
  as a jump. At 260px over 650ms the fastest frame is about 10px.
*/
const SNAP_RADIUS_PX = 260;
const SNAP_MS = 650;
const SNAP_EPSILON_PX = 2;
const SCROLL_END_DEBOUNCE_MS = 150;
/*
  How far *past* the gallery stop it still snaps back. Small, unlike the
  radius above: past that point you are inside the pinned track, where every
  pixel of scroll is advancing the photographs, and pulling back to the
  wordmark would undo scrolling the visitor meant.
*/
const SNAP_GALLERY_OVERSHOOT_PX = 80;
...
/*
  ...
  the answer — which is how a correctly-centred frame can be made to look
  40px off. Layout offsets ignore transforms, so they describe where the
...
export default function Home() {
  ...
  /*
    Snap to the four stops, once scrolling has stopped.

    Two things about this are deliberate, and both are lessons from attempts
    that were reverted:

    Nothing here is ever cancelled. Every listener is passive; no wheel or
    touch event is intercepted. An earlier version cancelled wheel events so
    it could drive the scroll itself, which meant the browser had to wait on
    JS before every scroll — that was the lag — and a held trackpad, which
    never falls silent, could be locked out entirely. Passive listeners make
    both of those impossible rather than unlikely.

    The travel is our own rAF animation, not `scrollTo({behavior:"smooth"})`.
    The browser's smooth scroll is fast and lands hard, and no radius makes
    that read as anything but a jump. Owning the curve is the whole
    difference between being pulled and gliding.

    Any hands-on input abandons an animation in progress — the visitor always
    wins — and when they next come to rest the same rule applies again.
  */
  useEffect(() => {
    ...
    /*
      Two pieces of state, and both exist to guarantee this terminates.

      `gliding` keeps our own landing from being read as the visitor coming to
      rest, which would start a second glide from the first one's finish.

      `attempted` is the target of the last glide, and it is not tried again
      until the visitor moves the page themselves. Without it a target that
      cannot actually be reached — clamped by the document's bounds, or moved
      by layout arriving late — is re-attempted on every landing, forever.
      That is not hypothetical: an earlier draft of this pinned the page at a
      fixed position and overrode everything, including deliberate scrolling.
    */
    let gliding = false;
    let attempted: number | null = null;
    // Where the last glide actually came to rest, so a later settle can tell
    // "still sitting where we left them" from "the page has moved since".
    let landedAt: number | null = null;

    const abort = () => {
      cancelAnimationFrame(raf);
      gliding = false;
      // A hand on the page is a fresh start: whatever we gave up on before is
      // allowed to be tried again.
      attempted = null;
    };

    const glideTo = (target: number) => {
      // Always cancel first. Two loops running at once would fight frame for
      // frame, and only the newer one's handle is the one `abort` holds.
      cancelAnimationFrame(raf);
      ...
      const step = (now: number) => {
        const t = Math.min((now - started) / SNAP_MS, 1);
        window.scrollTo(0, from + distance * snapEase(t));
        if (t < 1) {
    ...
    /*
      Measured on every settle rather than cached: both depend on viewport
      height and on layout that arrives late (fonts, images), and a settle is
      far too rare for the reads to cost anything.

      Each stop carries its own window, because the gallery's cannot be
      symmetric — see SNAP_GALLERY_OVERSHOOT_PX.
    */
    const stops = () => {
      ...
      document
        ...
        .forEach((f) =>
          out.push({
            at: frameCenterScrollTop(f),
            before: SNAP_RADIUS_PX,
            after: SNAP_RADIUS_PX,
          }),
        );
      ...
      if (beyond != null) {
        out.push({
          at: beyond,
          before: SNAP_RADIUS_PX,
          after: SNAP_GALLERY_OVERSHOOT_PX,
        });
      }
      return out;
    };
    ...
    const settle = () => {
      ...
      // Our own animation is still running; its landing is not a settle.
      if (gliding) return;

      const y = window.scrollY;
      // Moved since our last landing, by whatever means — a wheel, the nav, a
      // scrollbar drag. Fresh situation, so a target we gave up on is fair
      // game again.
      if (landedAt === null || Math.abs(y - landedAt) > SNAP_EPSILON_PX) {
        attempted = null;
      }
      ...
      for (const stop of stops()) {
        // Clamped, so a stop past the end of the document does not read as
        // unreachably far away and quietly disable itself.
        const at = Math.min(Math.max(stop.at, 0), max);
        const gap = at - y;
        // Outside this stop's own window in the direction it lies.
        if (gap > 0 ? gap > stop.before : -gap > stop.after) continue;
        if (best === null || Math.abs(gap) < Math.abs(best - y)) best = at;
      }

      if (best === null || Math.abs(best - y) <= SNAP_EPSILON_PX) return;
      // Already tried this one and it did not take. Leave it be until the
      // visitor moves the page themselves.
      if (attempted !== null && Math.abs(best - attempted) <= SNAP_EPSILON_PX) {
        return;
      }
      glideTo(best);
    };
    ...
    /*
      `scrollend` fires once momentum has fully died, which is exactly the
      moment wanted; Safari does not have it yet, so there a debounce on
      `scroll` says the same thing a beat later. Our own animation re-triggers
      this on landing, where the remaining gap is under SNAP_EPSILON_PX and it
      stops of its own accord.
    */
    const onScroll = () => {
      clearTimeout(debounce);
      debounce = setTimeout(settle, SCROLL_END_DEBOUNCE_MS);
    };
    const hasScrollEnd = "onscrollend" in window;
    if (hasScrollEnd) window.addEventListener("scrollend", settle);
    else window.addEventListener("scroll", onScroll, { passive: true });

    // Passive, every one of them: these observe the visitor taking over, they
    // never stand in the way of it.
